...
class TestCheckpointManager:
    def test_empty_dir_snapshot(self, mgr, tmp_work):
        """Snapshot of an empty dir should work (or return None)."""
        result = mgr.ensure_snapshot(tmp_work)
        # Empty dir might not create a commit, that's OK
        assert result is None or isinstance(result, str)

    def test_snapshot_creates_store(self, tmp_path, tmp_work):
        ...
        m.ensure_snapshot(tmp_work)
        assert Path(store).exists()

    def test_snapshot_with_files(self, mgr, tmp_work):
        """Snapshot should capture file state."""
        Path(tmp_work, "hello.txt").write_text("world")
        snap_id = mgr.ensure_snapshot(tmp_work)
        assert snap_id is not None
        assert len(snap_id) == 12

    def test_dedup_snapshot(self, mgr, tmp_work):
        """Second snapshot with no changes should return same ID."""
        Path(tmp_work, "a.txt").write_text("aaa")
        snap1 = mgr.ensure_snapshot(tmp_work)
        snap2 = mgr.ensure_snapshot(tmp_work)
        assert snap1 == snap2

    def test_list_snapshots(self, mgr, tmp_work):
        """list_snapshots should return entries."""
        Path(tmp_work, "f1.txt").write_text("one")
        mgr.ensure_snapshot(tmp_work)
        Path(tmp_work, "f2.txt").write_text("two")
        mgr.ensure_snapshot(tmp_work)

        snapshots = mgr.list_snapshots(tmp_work)
        assert len(snapshots) == 2
        assert all("id" in s and "date" in s and "message" in s for s in snapshots)

    def test_list_snapshots_empty(self, mgr, tmp_work):
        """No snapshots yet → empty list."""
        assert mgr.list_snapshots(tmp_work) == []

    def test_restore_full(self, mgr, tmp_work):
        ...
        # Restore
        result = mgr.restore(tmp_work, snap_id)
    ...
    def test_restore_single_file(self, mgr, tmp_work):
    ...
    def test_diff(self, mgr, tmp_work):
        ...
        diff_text = mgr.diff(tmp_work, snap_id)
        assert "v1" in diff_text or "v2" in diff_text

    def test_diff_no_changes(self, mgr, tmp_work):
    ...
    def test_prune_old_snapshots(self, tmp_path):
        ...
        for i in range(5):
            ...
            m.ensure_snapshot(work)

        snapshots = m.list_snapshots(work)
        assert len(snapshots) <= 3

    def test_multiple_dirs_isolated(self, mgr, tmp_path):
        ...
        mgr.ensure_snapshot(work1)

        Path(work2, "b.txt").write_text("bbb")
        mgr.ensure_snapshot(work2)

        snaps1 = mgr.list_snapshots(work1)
        snaps2 = mgr.list_snapshots(work2)
        assert len(snaps1) == 1
        assert len(snaps2) == 1
        assert snaps1[0]["id"] != snaps2[0]["id"]

    def test_snapshot_excludes_patterns(self, mgr, tmp_work):
